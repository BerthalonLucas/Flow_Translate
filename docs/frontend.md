...
`src/` contient le client React partagé par les trois fenêtres Tauri : `overlay`, `halo` (le balayage sur les lignes de la sélection pendant le travail, `src/halo/`, lot 6) et `settings`. Il ne contacte jamais un modèle directement : chaque effet passe par `src/bridge.ts`, qui applique les noms et formes de `docs/BRIDGE.md`.
...
L’overlay enregistre d’abord ses listeners, puis appelle `frontend_ready()`. Cette commande doit retourner le `Capture` en attente ou `null`; elle ne doit jamais relancer une capture de presse-papiers. Le halo et les réglages ne démarrent ni n’écoutent une traduction.
