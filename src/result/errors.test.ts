...
import { describeError, errorFamily, errorKindOf, errorKinds, type ErrorKind } from './errors';
...
describe('error kinds', () => {
  it('lists the codes of lot 10 and those the audit added, each in one family', () => {
    expect(errorKinds).toEqual(['unreachable', 'timeout', 'unauthorized', 'model_not_found', 'bad_endpoint', 'busy', 'length', 'stream_broken', 'paste_blocked', 'target_changed', 'not_editable', 'too_long', 'cancelled', 'server_error', 'no_selection', 'protected_field', 'keys_held', 'internal']);
    const families = Object.fromEntries(errorKinds.map(kind => [kind, errorFamily(kind)]));
  ...
  it('reads a code from the bridge, anything unknown being internal', () => {
    expect(errorKindOf('busy')).toBe('busy');
    for (const value of ['Busy', 'http_500', '', null, undefined, 42, { kind: 'busy' }]) expect(errorKindOf(value)).toBe('internal');
  });

  it('gives configuration errors the exact field of the Settings, for the request’s profile', () => {
    const fields: Array<[ErrorKind, string]> = [['unreachable', 'endpoint'], ['bad_endpoint', 'endpoint'], ['unauthorized', 'apiKey'], ['model_not_found', 'model']];
  ...
  it('takes the lab’s wording where the lab has it', () => {
    const mapped: Array<[string, ErrorKind]> = [['endpoint', 'unreachable'], ['key', 'unauthorized'], ['model', 'model_not_found'], ['busy', 'busy'], ['paste', 'paste_blocked'], ['changed', 'target_changed'], ['long', 'too_long']];
