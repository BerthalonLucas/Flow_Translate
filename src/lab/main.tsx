...
import { ilotScenarioFrom, ilotScenarios, scenarios, scenarioFrom } from './scenarios';
import { BugWorkbench } from './bugs';
import './workbench.css';

const labScenarioFrom = (value: string | null) => ilotScenarioFrom(value) ?? scenarioFrom(value);
...
function Workbench() {
  ...
  return <main className="workbench">
    ...
    <div className="workspace">
      <nav aria-label="Scénarios"><a href="/lab.html">← Défauts signalés</a><span className="section-label">ÉTATS TECHNIQUES</span>{scenarios.map(item => <button key={item.id} aria-current={scenario === item.id ? 'page' : undefined} onClick={() => { setScenario(item.id); remember({ scenario: item.id }); }}><span>{item.label}</span><small>{item.issue}</small></button>)}<span className="section-label">ÎLOT (LOT 7)</span>{ilotScenarios.map(item => <button key={item.id} aria-current={scenario === item.id ? 'page' : undefined} onClick={() => { setScenario(item.id); remember({ scenario: item.id }); }}><span>{item.label}</span><small>{item.issue}</small></button>)}<p className="missing">Le repli au survol n’existe pas encore dans l’application. Ce scénario montre la capsule actuelle, sans simuler une correction.</p></nav>
