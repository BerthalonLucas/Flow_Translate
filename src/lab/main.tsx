...
function Workbench() {
  ...
  const [size, setSize] = useState(['900x600', '760x640', '480x640', '1280x800'].includes(requestedSize) ? requestedSize : '900x600');
  ...
  return <main className="workbench">
    ...
    <div className="workspace">
      <nav aria-label="Scénarios"><a href="/lab.html">← Défauts signalés</a><span className="section-label">ÉTATS TECHNIQUES</span>{scenarios.map(item => <button key={item.id} aria-current={scenario === item.id ? 'page' : undefined} onClick={() => { setScenario(item.id); remember({ scenario: item.id }); }}><span>{item.label}</span><small>{item.ref}</small></button>)}<p className="missing">Onze scénarios, vingt-deux références. La capsule en est sortie : elle n’est plus affichée et le design system ne la dessine pas.</p></nav>
      <section className="review" aria-label="Zone de revue">
        <div className="controls"><label>Fenêtre<select value={size} onChange={e => { setSize(e.target.value); remember({ size: e.target.value }); }}><option value="900x600">900 × 600</option><option value="760x640">760 × 640 — Réglages</option><option value="480x640">480 × 640</option><option value="1280x800">1280 × 800</option></select></label><label>Décor derrière le verre<select value={theme} onChange={e => { setTheme(e.target.value); remember({ theme: e.target.value }); }}><option value="dark">Sombre</option><option value="light">Clair</option></select></label><label className="check"><input type="checkbox" checked={reduced} onChange={e => { setReduced(e.target.checked); remember({ motion: e.target.checked ? 'reduce' : 'full' }); }}/>Mouvements réduits</label><button className="replay" onClick={() => setRun(run + 1)}><RotateCcw size={15}/>Rejouer</button><a href={src} target="_blank" rel="noreferrer" aria-label="Ouvrir le scénario seul"><ExternalLink size={17}/></a></div>
        <div className="canvas"><iframe key={`${src}-${run}`} title="Scénario FlowTranslate" src={src} width={width} height={height}/></div>
        <footer><strong>{selected.ref} · {selected.label}</strong><p>{selected.expected}</p><small>Taille réelle, sans mise à l’échelle. Le sélecteur ci-dessus ne change que le décor derrière le verre : les Réglages suivent le thème Windows de ce navigateur. Ce rendu web ne valide ni le dépoli du bureau ni le focus Windows.</small></footer>
      </section>
    </div>
  </main>;
}
