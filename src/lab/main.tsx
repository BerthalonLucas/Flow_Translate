...
function Workbench() {
  ...
  return <main className="workbench">
    ...
    <div className="workspace">
      <nav aria-label="Scénarios"><a href="/lab.html">← Défauts signalés</a><span className="section-label">ÉTATS TECHNIQUES</span>{scenarios.map(item => <button key={item.id} aria-current={scenario === item.id ? 'page' : undefined} onClick={() => { setScenario(item.id); remember({ scenario: item.id }); }}><span>{item.label}</span><small>{item.issue}</small></button>)}</nav>
