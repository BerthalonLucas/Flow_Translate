...
On Windows, `powershell -ExecutionPolicy Bypass -File server/start.ps1 -Profile fast -HealthDeadlineSeconds 900` performs preflight, validates Compose, starts only the selected service, and waits for health. On timeout it returns an error and leaves that container running for inspection; it never stops unrelated workloads. Repeat with `quality` only after the Fast trial.
