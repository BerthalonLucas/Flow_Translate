            ...
            "capture" | "capture-copy" => {
                ...
                match capture {
                    Ok(capture) => {
                        ...
                        serde_json::json!({"ok": true, "replaceable": target.is_some(), "origin": capture.public.origin, "selected_length": capture.public.text.replace("\r\n", "\n").replace('\r', "\n").chars().count(), "document_length": target.as_ref().and_then(|t| t.document.as_ref().map(|d| d.chars().count())),
