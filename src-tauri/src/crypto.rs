...
pub fn protect(data: &[u8]) -> Result<Vec<u8>, String> {
    ...
    let input = CRYPT_INTEGER_BLOB {
        cbData: data.len().try_into().map_err(|_| "Donnée trop volumineuse.".to_string())?,
...
pub fn unprotect(data: &[u8]) -> Result<Vec<u8>, String> {
    use windows::Win32::{Foundation::LocalFree, Security::Cryptography::*};
    let input = CRYPT_INTEGER_BLOB {
        cbData: data.len().try_into().map_err(|_| "Donnée chiffrée invalide.".to_string())?,
        pbData: data.as_ptr() as *mut u8,
    };
    let mut output = CRYPT_INTEGER_BLOB::default();
    unsafe {
...
#[cfg(not(windows))]
pub fn protect(_data: &[u8]) -> Result<Vec<u8>, String> { Err("DPAPI nécessite Windows.".into()) }
#[cfg(not(windows))]
pub fn unprotect(_data: &[u8]) -> Result<Vec<u8>, String> { Err("DPAPI nécessite Windows.".into()) }
